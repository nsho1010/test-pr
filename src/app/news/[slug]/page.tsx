  ...
  const options: HTMLReactParserOptions = {
    replace: domNode => {
      ...
      if (domNode instanceof Element && domNode.name === 'pre') {
        if (domNode.children[0] instanceof Element && domNode.children[0].name === 'code' && domNode.children[0].children[0] instanceof Text) {
    ...
    <main className="max-w-prose mx-auto space-y-8">
      ...
      <article className="prose prose-slate mx-auto dark:prose-invert">
        <div className="space-y-4">
          <p className="text-muted-foreground">{formatDate(data.publishedAt || data.createdAt)}</p>  
