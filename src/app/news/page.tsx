...
const NewsListPage = async ({searchParams}: {searchParams: {page: number, pageSize: number}}) => {
  ...
  return (
    <main className="flex flex-col max-w-3xl mx-auto gap-4 lg:gap-8">
      ...
      <h1 className="text-4xl font-bold tracking-tight underline decoration-primary decoration-1 underline-offset-8">お知らせ</h1>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {data.contents.map((news) => (
            <Link key={news.id} href={`/news/${news.id}`}>
              <Card key={news.id}>
                <CardHeader>
                  <time className="text-sm text-muted-foreground" dateTime={news.publishedAt || news.createdAt}>
